...
% Define a constant depth for the points
depth = 50; % Adjust this value as needed

% Initialize an empty matrix to store the 3D points
pointCloudData = [];

% Loop through each detected line
for i = 1:numel(detectedLines)
    % Get the detected lines for this frame
    lines = detectedLines{i};
    
    % Convert each line to 3D points with the specified depth
    for k = 1:length(lines)
        % Extract endpoints of the line
        point1 = [lines(k).point1, depth];
        point2 = [lines(k).point2, depth];
        
        % Append the endpoints to the point cloud data matrix
        pointCloudData = [pointCloudData; point1; point2];
    end
end
